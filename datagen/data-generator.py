...
# Original code by Harry Schwartz
...
def genSentence(markovLength):
    ...
    while(len(sent) < 50 or len(sent) > 150):
...
def buildSentence(num, sent):
    p = inflect.engine()
    #line = "\"" + str(num) + "\", \""
    line = "\"" + str(p.number_to_words(num)) + "\", \""
    line += sent
    line += "\"\n"
    return line
...
def main():
	...
	sentence = genSentence(markovLength)
        newSentence = sentence
	numSameCode = 0
	while(numSameCode < 20):
            line = buildSentence(codeNum,newSentence)
            #print(line)
            f.write(line)
